...
- [[#misc-keywords][Misc keywords]]
  - [[#minor-mode-keyword][:minor-mode keyword]]
...
* Misc keywords
** :minor-mode keyword
~:minor-mode~ keyword provides a front end to easily enable minor mode.
Minor-mode is followed by ~-mode~ and has the custom of being activated by passing a ~1~,
...
If the latter requires ~t~ to be valid, the value can be explicitly specified using the ~cdr~ of the cons cell.
